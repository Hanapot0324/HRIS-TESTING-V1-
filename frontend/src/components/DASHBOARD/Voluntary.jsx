...
@keyframes voluntaryShimmer {
  0%   { background-position: -800px 0; }
  100% { background-position:  800px 0; }
}
@keyframes voluntaryPulse {
  0%, 100% { opacity: 1; }
  50%       { opacity: 0.60; }
}
`;
...
const Bone = ({ w = '100%', h = 14, r = 6, sx = {} }) => (
  <Box
    sx={{
      ...
      animation: 'voluntaryShimmer 1.6s infinite linear',
      flexShrink: 0,
      ...sx,
    }}
  />
    ...
    >
      <Box
        sx={{
          ...
          animation: 'voluntaryPulse 2s ease-in-out infinite',
        }}
      >
        <Box
          sx={{
          ...
          <Grid item xs={12} lg={col === 0 ? 4 : 8} key={col}>
            <Box
              sx={{
                ...
                bgcolor: '#fff',
                overflow: 'hidden',
                animation: `voluntaryPulse 2s ease-in-out ${col * 0.1}s infinite`,
...
const formatDateRange = (dateFrom, dateTo) => {
  ...
  if (!dateFrom) return `Until ${formatDateDisplay(dateTo)}`;
  if (!dateTo) return `From ${formatDateDisplay(dateFrom)}`;
...
const VoluntaryWork = () => {
  ...
  const { hasAccess } = usePageAccess('voluntarywork');
  ...
  useEffect(() => {
    fetchVoluntaryWork().finally(() => setPageLoading(false));
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    refreshRef.current = fetchVoluntaryWork;
  });

  useEffect(() => {
    if (!socket || !connected) return;
    const handler = () => refreshRef.current?.();
    socket.on('voluntaryWorkChanged', handler);
    return () => socket.off('voluntaryWorkChanged', handler);
  }, [socket, connected]);
  ...
  const fetchVoluntaryWork = async () => {
    ...
    try {
      ...
      showSnackbar('Failed to fetch voluntary work records.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredData = useMemo(() => {
    ...
    return groupedData.filter(
      (g) =>
        ...
        g.records.some(
          (r) =>
            (r.nameAndAddress || '').toLowerCase().includes(q) ||
            (r.natureOfWork || '').toLowerCase().includes(q)
        )
    );
  ...
  const validateAdd = () => {
    const errs = {};
    if (!newVoluntary.person_id) errs.person_id = 'Required';
    if (!newVoluntary.nameAndAddress?.trim()) errs.nameAndAddress = 'Required';
    if (!newVoluntary.dateFrom) errs.dateFrom = 'Required';
    if (!newVoluntary.dateTo) errs.dateTo = 'Required';

    if (newVoluntary.dateFrom && newVoluntary.dateTo) {
      if (new Date(newVoluntary.dateFrom) > new Date(newVoluntary.dateTo)) {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      showSnackbar(err.response?.data?.error || 'Failed to add voluntary work record.', 'error');
      setLoading(false);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    if (tempRecordData.dateFrom && tempRecordData.dateTo) {
      if (new Date(tempRecordData.dateFrom) > new Date(tempRecordData.dateTo)) {
    ...
    try {
      await axios.put(
        `${API_BASE_URL}/VoluntaryRoute/voluntary-work/${tempRecordData.id}`,
        tempRecordData,
        getAuthHeaders()
      );
      ...
      showSnackbar('Failed to update voluntary work record.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    if (!window.confirm('Are you sure you want to delete this voluntary work record?')) {
      return;
    }
    ...
    try {
      await axios.delete(
        `${API_BASE_URL}/VoluntaryRoute/voluntary-work/${selectedRecord.id}`,
        getAuthHeaders()
      );
      ...
      showSnackbar('Failed to delete voluntary work record.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const canAdd =
    !loading &&
    !!newVoluntary.person_id &&
    !!newVoluntary.nameAndAddress?.trim() &&
    !!newVoluntary.dateFrom &&
    !!newVoluntary.dateTo;
  ...
  if (hasAccess === false) {
    return (
      <AccessDenied
        title="Access Denied"
        message="You do not have permission to access Voluntary Work Information."
  ...
  if (pageLoading) return <VoluntaryWireframe />;
  ...
  return (
    <>
      ...
      <Fade in timeout={400}>
        ...
        >
          <LoadingOverlay open={loading} message="Processing voluntary work record..." />
          <SuccessfulOverlay
            open={successOpen}
          ...
          <SectionCard sx={{ mb: 2, overflow: 'hidden' }}>
            ...
            >
              ...
              >
                <VolunteerActivismIcon sx={{ fontSize: 32, color: T.accent }} />
                <Box>
                  <Typography
                    sx={{
                      fontSize: '1.25rem',
                  ...
                  >
                    Administrative Panel • Add and manage voluntary work records
                  </Typography>
                </Box>
              </Box>
              <Box
                ...
                >
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <EmployeeAutocomplete
                      ...
                      onChange={(val) => {
                        setNewVoluntary((r) => ({ ...r, person_id: val }));
                        setErrors((e) => {
                          const n = { ...e };
                          delete n.person_id;
                          return n;
                  ...
                  <FormSectionLabel icon={VolunteerActivismIcon}>Voluntary Work Details</FormSectionLabel>

                  <Box sx={{ mb: 2 }}>
                    <Typography
                      sx={{ fontSize: '0.75rem', fontWeight: 600, color: T.accent, mb: 0.75 }}
                    >
                      Name and Address <Box component="span" sx={{ color: '#c62828' }}>*</Box>
                    </Typography>
                    <FieldInput
                      value={newVoluntary.nameAndAddress}
                      onChange={(e) => {
                        setNewVoluntary((r) => ({ ...r, nameAndAddress: e.target.value }));
                        setErrors((er) => {
                          const n = { ...er };
                          delete n.nameAndAddress;
                          return n;
                        });
                      }}
                      fullWidth
                      size="small"
                      error={!!errors.nameAndAddress}
                      helperText={errors.nameAndAddress || ''}
                    />
                  </Box>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <FieldInput
                      ...
                      onChange={(e) => {
                        setNewVoluntary((r) => ({ ...r, dateFrom: e.target.value }));
                        setErrors((er) => {
                          const n = { ...er };
                          delete n.dateFrom;
                          return n;
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <FieldInput
                      ...
                      onChange={(e) => {
                        setNewVoluntary((r) => ({ ...r, dateTo: e.target.value }));
                        setErrors((er) => {
                          const n = { ...er };
                          delete n.dateTo;
                          return n;
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <FieldInput
                      ...
                      onChange={(e) => setNewVoluntary((r) => ({ ...r, numberOfHours: e.target.value }))}
                      fullWidth
                      size="small"
                    />
                  </Box>
                  ...
                  <Box sx={{ mb: 2 }}>
                    ...
                    <FieldInput
                      ...
                      onChange={(e) => setNewVoluntary((r) => ({ ...r, natureOfWork: e.target.value }))}
                      fullWidth
                      size="small"
                    />
                  </Box>
                  ...
                  <Box sx={{ mt: 'auto' }}>
                    {(selectedEmployee || newVoluntary.nameAndAddress) && (
                      <AccentButton
                        onClick={() => {
                          setNewVoluntary({
                            nameAndAddress: '',
                    ...
                    >
                      {loading ? 'Adding...' : 'Add Voluntary Work'}
                    </AccentButton>
                  </Box>
                </Box>
              </SectionCard>
            ...
            <Grid item xs={12} lg={8}>
              ...
              >
                ...
                >
                  ...
                  >
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                      ...
                      <Typography sx={{ fontSize: '0.88rem', fontWeight: 700, color: T.text }}>
                        Employee Voluntary Work Records
                      </Typography>
                    </Box>
                  ...
                  <FieldInput
                    size="small"
                    placeholder="Search by employee ID, name, organization, or nature of work..."
                    value={searchTerm}
                ...
                >
                  {pagedData.length === 0 ? (
                    <Box sx={{ py: 10, textAlign: 'center' }}>
                      ...
                      >
                        <VolunteerActivismIcon sx={{ fontSize: 32, color: alpha(T.accent, 0.3) }} />
                      </Box>
                      <Typography
                      ...
                      <Typography sx={{ fontSize: '0.78rem', color: T.faint }}>
                        {groupedData.length === 0
                          ? 'Use the form on the left to add a voluntary work record.'
              ...
              >
                ...
                >
                  ...
                  >
                    ...
                    >
                      <VolunteerActivismIcon sx={{ fontSize: 18, color: '#fff' }} />
                    </Box>
                    <Box>
                      <Typography
                        sx={{
                      ...
                      >
                        Voluntary Work Records of {modalState.employeeName}
                      </Typography>
                ...
                <Box sx={{ flexGrow: 1, display: 'flex', overflow: 'hidden' }}>
                      ...
                      {modalState.records.map((record) => (
                        ...
                        >
                          ...
                          >
                            ...
                            >
                              {record.nameAndAddress?.charAt(0)?.toUpperCase() || '?'}
                            </Avatar>
                            <Box sx={{ minWidth: 0 }}>
                              <Typography
                  ...
                  <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
                    {selectedRecord ? (
                          ...
                          {!isEditingRecord ? (
                            <>
                              ...
                              >
                                ...
                                >
                                  {selectedRecord.nameAndAddress?.charAt(0)?.toUpperCase() || '?'}
                                </Avatar>
                                <Typography
                                  sx={{
                                    fontSize: '1.1rem',
                                ...
                                >
                                  {selectedRecord.nameAndAddress || 'Unnamed Organization'}
                                </Typography>
                                <Typography
                                  sx={{ fontSize: '0.78rem', color: T.accentMid, mt: 0.5 }}
                                >
                                  {formatDateRange(selectedRecord.dateFrom, selectedRecord.dateTo)}
                                </Typography>
                              </Box>

                              <Box sx={{ px: 3.5, py: 2.5 }}>
                                {[
                                  { label: 'Name and Address', value: selectedRecord.nameAndAddress },
                                  { label: 'Date From', value: formatDateDisplay(selectedRecord.dateFrom) },
                                  { label: 'Date To', value: formatDateDisplay(selectedRecord.dateTo) },
                                  {
                                    label: 'Number of Hours',
                                    value: selectedRecord.numberOfHours || '-',
                                  },
                                  {
                                    label: 'Nature of Work',
                                    value: selectedRecord.natureOfWork || '-',
                                  },
                            ...
                            <Box sx={{ px: 3.5, py: 3 }}>
                              ...
                              <Box sx={{ mb: 2 }}>
                                ...
                                <FieldInput
                                  value={tempRecordData.nameAndAddress || ''}
                                  onChange={(e) =>
                                    setTempRecordData((r) => ({
                                      ...r,
                              ...
                              <Box sx={{ mb: 2 }}>
                                ...
                                <FieldInput
                                  type="date"
                                  value={tempRecordData.dateFrom?.split('T')[0] || ''}
                                  onChange={(e) =>
                                    setTempRecordData((r) => ({ ...r, dateFrom: e.target.value }))
                                  }
                                  fullWidth
                                  size="small"
                                  InputLabelProps={{ shrink: true }}
                              ...
                              <Box sx={{ mb: 2 }}>
                                ...
                                <FieldInput
                                  type="date"
                                  value={tempRecordData.dateTo?.split('T')[0] || ''}
                                  onChange={(e) =>
                                    setTempRecordData((r) => ({ ...r, dateTo: e.target.value }))
                                  }
                                  fullWidth
                                  size="small"
                                  InputLabelProps={{ shrink: true }}
                              ...
                              <Box sx={{ mb: 2 }}>
                                ...
                                <FieldInput
                                  value={tempRecordData.numberOfHours || ''}
                                  onChange={(e) =>
                                    setTempRecordData((r) => ({
                                      ...r,
                              ...
                              <Box sx={{ mb: 2 }}>
                                ...
                                <FieldInput
                                  value={tempRecordData.natureOfWork || ''}
                                  onChange={(e) =>
                                    setTempRecordData((r) => ({
                                      ...r,
                      ...
                      >
                        <VolunteerActivismIcon sx={{ fontSize: 40, color: alpha(T.accent, 0.2) }} />
                        <Typography
